...
export default function AdminHome() {
    ...
    const [form, setForm] = useState({ kind: 'image', url: '', caption: '', active: true });
    ...
    const submit = async (e) => {
        e.preventDefault();
        await apiFetch('/admin/posts', { method: 'POST', body: form });
        setForm({ kind: 'image', url: '', caption: '', active: true });
        load();
    };
            ...
            <div className="bg-white/5 rounded-2xl p-6 border border-white/10 shadow-lg">
                ...
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="text-white/80 text-sm font-medium">Upload image</label>
                        <div className="flex gap-2">
                            ...
                            <input
                                ...
                                className="flex-1 px-3 py-2 rounded-lg bg-white/10 text-white border border-white/20 placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-amber-500"
                            />
                        </div>
                    </div>
                    ...
                    <button
                        type="submit"
                        className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white px-6 py-3 rounded-lg font-semibold hover:from-amber-600 hover:to-orange-600 transition-all duration-200 shadow-lg"
                    >
                        Post Announcement
                    </button>
                </form>
                ...
                <div className="space-y-2">
                    {posts.map(p => (
                        <div key={p._id} className="p-4 rounded-lg bg-white/5 border border-white/10 text-white">
                            ...
                            <div className="truncate opacity-90 text-sm mb-2">{p.url}</div>
